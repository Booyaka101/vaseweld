...
- **111 tests pass** with `python -m pytest`, about 30 seconds. That includes a matrix that welds
...
- **No physical print.** The Klipper simulation in `sim/` is the closest available substitute: real
  firmware, real motion planner, real extruder limits, no hardware. It proves the printer would
  accept the file and that the hand splice it replaces would be rejected. It cannot prove layer
  adhesion or surface finish. Only a print can.
...
4. `git tag v1.0.0 && git push --tags` fires `.github/workflows/release.yml`, which builds
   ...
   commit.
5. Best first distribution step: comment on
